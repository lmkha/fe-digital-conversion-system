...
import { findRoleById } from '@/services/role';
...
export function EditRoleModal({ isOpen, roleId, onClose, onSubmitted }: AddRoleModalProps) {
            ...
            if (roleId) {
                ...
                if (roleResult.success) {
                    setRoleData({
                        ...
                        permissionIds: roleResult.data?.permissionIds,
                    });
                    setSubmitData({
                        roleId: roleResult.data?.roleId,
                        roleName: roleResult.data?.roleName,
                ...
                // After findRoleById completed, get permission list
                const permissionResult = await getPermissionByFilter('', '', '', '');
                if (permissionResult.success) {
                    ...
                    setPermissionList(permissionResult.parentList.map((item: {
                        ...
                        const children = item.childrenList.map((component: { permissionId: string; permissionCode: string; permissionName: string }) => ({
                            ...
                            isCheck: roleResult.data?.permissionIds.includes(component.permissionId),
                        }));

                        // Check if all children are checked, then check item
                        const isAllChildrenChecked = children.every((child: { isCheck: boolean }) => child.isCheck === true);

                        return {
                            id: item.permissionId,
                            code: item.permissionCode,
                            name: item.permissionName,
                            isCheck: isAllChildrenChecked || roleResult.data?.permissionIds.includes(item.permissionId), // item cũng được check nếu tất cả children được check
                            children: children
                        };
                    }));
                }
                    ...
                    <Button
                        ...
                        onClick={() => {
                            role.updateRole({
                                ...
                                permissionIds: submitData.permissionIds,
                            }).then((result) => {
                                onSubmitted(result.success, result.message);
                                if (result.success) {
                                    onClose();
                                }
                            });
