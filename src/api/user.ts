...
class UserAPI extends Base {
  ...
  async checkUserName(userName: string) {
    try {
      const response = await this.post("/user/checkUserName", {
        userName: userName,
      });
      return {
        success: true,
  ...
  async importUsers(users: PreviewUserList) {
    try {
      const url = `/user/import-user`;

      const response = await this.post(url, { users: users });
  ...
  async previewUser(file: File, deptId: string) {
    try {
      const url = `/user/preview-user?deptId=${deptId}`;
      const formData = new FormData();
      formData.append("file", file);
  ...
  async validateUser(users: PreviewUserList) {
    try {
      const url = `/user/validate-users-import`;
  ...
  async downLoadUserError(listUserError: PreviewUserList, deptId: string) {
    try {
      const url = `/user/download-users-error?deptId=${deptId}`;
